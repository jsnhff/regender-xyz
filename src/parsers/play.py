...
class PlayParser:
            ...
            # Parse scene content
                ...
                if line.startswith('[') and line.endswith(']'):
                    current_scene.elements.append(
                        PlayElement(
                            type=PlayElementType.STAGE_DIRECTION,
                            content=line[1:-1]
                        )
                    )
                ...
                # Check for character name (all caps, often followed by period or colon)
                    ...
                    while j < len(lines):
                        ...
                        if not next_line:
                            break
                        if (self._is_character_name(next_line) or 
                            self._is_stage_direction(next_line) or
                            self._is_act_marker(next_line) or
                            self._is_scene_marker(next_line)):
                            break
    ...
    def _is_play_element(self, line: str) -> bool:
        ...
        line_upper = line.upper().strip()
        return any(marker in line_upper for marker in [
    ...
    def _is_act_marker(self, line: str) -> bool:
        ...
        return (line_upper.startswith('ACT ') or 
                line_upper == 'ACT' or  # Sometimes just "ACT" alone
                line_upper.startswith('ACTUS ') or  # Latin
                (line_upper.startswith('THE ') and 'ACT' in line_upper))
    ...
    def _is_scene_marker(self, line: str) -> bool:
        ...
        line_upper = line.upper().strip()
        return (line_upper.startswith('SCENE ') or 
                line_upper.startswith('SC. ') or
                (line_upper.startswith('THE ') and 'SCENE' in line_upper))
    ...
    def _is_stage_direction(self, line: str) -> bool:
        ...
        line_stripped = line.strip()
        return (line_stripped.startswith('[') and line_stripped.endswith(']')) or \
               line_stripped.startswith('Enter ') or \
               line_stripped.startswith('Exit ') or \
               line_stripped.startswith('Exeunt')
    ...
    def _parse_prologue(self, lines: List[str], start_idx: int, play: Play) -> int:
        ...
        while i < len(lines):
            ...
            if (self._is_act_marker(line) or 
                self._is_scene_marker(line) or
                self._is_epilogue_marker(line)):
                break
    ...
    def _is_dramatis_personae(self, line: str) -> bool:
        ...
        line_lower = line.lower().strip()
        return ('dramatis' in line_lower and 'person' in line_lower) or \
               line_lower == 'characters' or \
               line_lower == 'persons represented'
