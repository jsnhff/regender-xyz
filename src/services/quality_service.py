...
from src.services.prompts import QC_REVIEW_PROMPT_TEMPLATE
...
class QualityService(BaseService):
    ...
    async def review_book(
        ...
        """Review and correct all paragraphs in the book."""
        ...
        for batch in batches:
            ...
            for (ch_idx, para_idx, _, original), corrected in zip(batch, corrected_texts):
                if corrected.strip() and corrected.strip() != original.strip():
                    corrections_made += 1
                    # Replace paragraph sentences with corrected text
                    corrected_book.chapters[ch_idx].paragraphs[para_idx] = Paragraph(
                        sentences=[corrected.strip()]
                    )

        quality_score = round(100.0 * (total - corrections_made) / total, 1) if total > 0 else 100.0

        self.logger.info(
            f"QC complete: {corrections_made}/{total} paragraphs corrected, "
