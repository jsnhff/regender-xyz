    ...
    async def process(self, transformation: Transformation) -> Transformation:
        ...
        try:
            ...
            if current_quality >= target_quality:
                ...
                self.logger.warning(
                    f"Quality target not met: {current_quality:.1f}/100 (target: {target_quality})"
                )
    ...
    async def _apply_corrections(
        ...
        for issue_type, issue_list in issues_by_type.items():
            if issue_type == "consistency":
                transformation = await self._fix_consistency_issues(transformation, issue_list)
            elif issue_type == "completeness":
                transformation = await self._fix_completeness_issues(transformation, issue_list)
            elif issue_type == "grammar":
                transformation = await self._fix_grammar_issues(transformation, issue_list)
            else:
