...
def print_summary(results: List[Dict[str, Any]]):
    ...
    if successful > 0:
        ...
        if chapters:
            print(f"  Chapters per book: min={min(chapters)}, max={max(chapters)}, avg={sum(chapters)/len(chapters):.1f}")
        
        if paragraphs:
            print(f"  Paragraphs per book: min={min(paragraphs)}, max={max(paragraphs)}, avg={sum(paragraphs)/len(paragraphs):.0f}")
...
def main():
    ...
    for i, book_name in enumerate(test_books, 1):
        ...
        if result['success']:
            status = "✅"
            info = f"{result['format']} ({result['confidence']:.0f}%), {result['chapters']} chapters"
            if result['issues']:
