...
NICKS = {"characters": [{"name": "Edward", "nicknames": {"Lizzy": "Ned", "Eliza": "Ned"}}]}
...
class TestTheNicknameIsAsked:
    ...
    async def test_a_pet_name_maps_to_a_pet_name(self):
        e = engine(NICKS)
        name_map, _ = await build(e, {"Elizabeth Bennet": "Edward Bennet"})
    ...
    async def test_the_formal_name_is_untouched(self):
        e = engine(NICKS)
        name_map, _ = await build(e, {"Elizabeth Bennet": "Edward Bennet"})
        assert name_map.get("Elizabeth Bennet") == "Edward Bennet"
    ...
    async def test_it_costs_one_call_for_the_whole_cast(self):
        ...
        await build(
            e,
            {"Elizabeth Bennet": "Edward Bennet", "Jane Bennet": "John Bennet"},
            cast(
...
class TestItCannotCostTheRun:
    ...
    async def test_a_failed_call_falls_back_to_the_formal_name(self):
        e = engine(fail=True)
        name_map, report = await build(e, {"Elizabeth Bennet": "Edward Bennet"})
        assert name_map.get("Elizabeth Bennet") == "Edward Bennet"
    ...
    async def test_an_invented_nickname_is_refused(self):
        ...
        e = engine({"characters": [{"name": "Edward", "nicknames": {"Lizzy": "Lizzy"}}]})
        name_map, _ = await build(e, {"Elizabeth Bennet": "Edward Bennet"})
    ...
    async def test_no_nickname_aliases_means_no_call(self):
        e = engine(NICKS)
        await build(
            e, {"Elizabeth Bennet": "Edward Bennet"}, cast(("Elizabeth Bennet", Gender.FEMALE, []))
        )
