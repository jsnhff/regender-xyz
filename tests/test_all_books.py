...
def main():
    ...
    for i, book_file in enumerate(book_files, 1):
        ...
        if result['success']:
            print(f"✓ {result['format']} ({result['confidence']:.0f}%) - {result['chapters']} chapters")
        else:
    ...
    if successful:
        ...
        print(f"\n📖 CONTENT STATISTICS:")
        if chapters:
            print(f"  Chapters per book: avg={sum(chapters)/len(chapters):.1f}, min={min(chapters)}, max={max(chapters)}")
        if paragraphs:
            print(f"  Paragraphs per book: avg={sum(paragraphs)/len(paragraphs):.0f}, min={min(paragraphs)}, max={max(paragraphs)}")
